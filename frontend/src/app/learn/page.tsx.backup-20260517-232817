    ...
    <div className="flex flex-wrap gap-2 p-3 t-panel border-t t-border">
      ...
      {answers.map(ans => (
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg border t-border t-panel transition-all ${
            ans.confidence === 'high' 
              ? 't-accent-text border-current font-semibold' 
              : 't-text hover:t-accent-text'
          }`}
        >
          {ans.text}
        </button>
...
export default function LearnPage() {
  ...
  const prevMessageContentRef = useRef<string>('')  // Phase 9A: 防止 answer_good 重复触发
  ...
  // Phase 9A: 触发 answer_good 反馈演出（检测 AI 正面反馈）
  useEffect(() => {
              ...
              {!stageComplete && (
                <div className="border-t t-border p-4 space-y-3 t-panel">
                  ...
                  <textarea
                    ...
                    placeholder={
                      interactionConfig.mode === 'PRESET_ONLY' && currentNode === 'practice'
                        ? '请选择上方预制答案' 
                        : interactionConfig.mode === 'FREE_INPUT_ONLY' && currentNode === 'practice'
                        ? '请手动输入答案（本题禁用预制答案）'
                        : awaitsInput ? "输入你的回答（Shift+Enter 换行）..." : "AI 正在讲解中..."
                    }
                    disabled={!awaitsInput || loading || (interactionConfig.mode === 'PRESET_ONLY' && currentNode === 'practice')}
                    rows={3}
                    className={`w-full t-input-field border rounded-xl px-4 py-3 text-sm resize-none disabled:opacity-40 transition-all ${
                      interactionConfig.mode === 'PRESET_ONLY' && currentNode === 'practice' ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  />
