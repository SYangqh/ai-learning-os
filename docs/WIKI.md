...
| 机制 | 说明 |
|------|------|
| AES-256-GCM | API Key 加密存储，明文不落库、不进日志 |
| JWT 双 Token | Access Token（短期）+ Refresh Token（长期），支持无感续签 |
| 部分唯一索引 | `uidx_user_llm_credentials(user_id, provider_key) WHERE revoked_at IS NULL`，防重复存活凭据 |
...
1. Fork 项目，创建特性分支：`git checkout -b feature/my-feature`
2. 遵循 [copilot-instructions.md](.github/copilot-instructions.md) 中的编码规范
3. 修改后必须运行验收脚本全部通过：`./verify.sh`（或 Windows 下 `verify.bat`）
4. 提交信息使用中文或英文均可，格式：`feat: 添加 XXX` / `fix: 修复 XXX`
5. 发起 Pull Request，描述改动目的和验收结果
